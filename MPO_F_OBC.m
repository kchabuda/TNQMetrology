function [result,A0,c] = MPO_F_OBC(figureofmerit,n,d,bdl,noiserange,integral0,integral1,integral2,lherm,imprecision,A0,c)
% Parameters
...
while 1
    ...
    for nx = 1:d
        for nxp = 1:d
            B{1}(:,:,nx,nxp) = kron([-1i*(nx-nxp),1],A{1}(:,:,nx,nxp));
            for x = 2:n-1
                B{x}(:,:,nx,nxp) = kron([1,0;-1i*(nx-nxp),1],A{x}(:,:,nx,nxp));
            end
            B{n}(:,:,nx,nxp) = kron([1;-1i*(nx-nxp)],A{n}(:,:,nx,nxp));
        end
    end
...
%  MPO (dual)
    ...
    for nx = 1:d
        for nxp = 1:d
            cp(1,:,nx,nxp,1) = kron([1i*(nx-nxp),1],ch(1,:,nx,nxp,1));
            for x = 2:n-1
                cp(:,:,nx,nxp,x) = kron([1,0;1i*(nx-nxp),1],ch(:,:,nx,nxp,x));
            end
            cp(:,1,nx,nxp,n) = kron([1;1i*(nx-nxp)],ch(:,1,nx,nxp,n));
        end
    end
    C2D = cell([1,n]);
    CPD = cell([1,n]);
