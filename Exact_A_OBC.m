function [result,qAvar] = Exact_A_OBC(m,d,ti,fullnoise,noiserange,alfa,gamma,beta,imprecision)
%d = 2;
%fullnoise = 0;
%noiserange = 2;
...
while 1
    ...
    if iter == 0 && sum(diag(rbareigval) < -10^-10) > 0
        result = NaN;
        return
    end
    lpart1 = zeros(d^n);
    ...
    lpd = (1i/sqrt(ta))*l.*sumintegral2.*expintegral2;
    eiginput = 2*lpd-l2d;
