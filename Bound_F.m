function result = Bound_F(numof2box,d,integral0,integral1)
if numof2box == 1/2
    n = 1;
else
    n = numof2box+1;
...
an1half = exp(-nxminusnxp.^2*integral0/4);
an1half = permute(an1half,[3,4,1,2]);
bdnoise2 = 2*d-1;
...
for nx = 1:d
    for nxp = 1:d
        an(:,:,nx,nxp,1) = an1half(:,:,nx,nxp)*an2(:,:,nx,nxp,1);
        an(:,:,nx,nxp,2) = an1(:,:,nx,nxp)*an2(:,:,nx,nxp,2);
        an(:,:,nx,nxp,3) = an1half(:,:,nx,nxp)*an2(:,:,nx,nxp,3);
    end
end
channel2 = zeros(d^n);
for nt = 1:d^n
